...
export class RippleRenderer {
  ...
  createForegroundRipple(
    ...
    radius: number,
    speedFactor: number,
    transitionEndCallback: (r: ForegroundRipple, e: TransitionEvent) => void): Element {
    ...
    rippleDiv.style.backgroundColor = window.getComputedStyle(rippleDiv).color || 'rgb(0,0,0)';
    // Start the ripple tiny.
    rippleDiv.style.transform = `scale(0.001)`;

    const fadeInSeconds = ((1 / (speedFactor) * 5 || 0.5)) * Math.max(
  ...
  fadeOutForegroundRipple(ripple: Element) {
    ...
    ripple.addEventListener('transitionend',
      (event: TransitionEvent) => {
        this.removeRippleFromDom(ripple);
      });
  }
